...
const actionForm = async (name) => {
  ...
  data.forEach((country) => {
    ...
    p.classList.add("mb-2", "mt-4", "fs-2", "fw-bold");
    const p2 = document.createElement("p");
    p2.classList.add("mt-2");
    const p3 = document.createElement("p");
    p3.classList.add("mx-auto", "mt-5");
    p3.style = "border: 2px solid black; width: 10vw; background-color: black";
    let lstLng = [];
    ...
    lstCur.forEach((cur) => {
      if (country.capital) {
        ...
        p2.innerHTML = `Capitale : ${country.capital[0]}  <br/>  
                        Habitants : ${country.population} <br/> 
                        Nom en francais : ${
                          country.translations.fra.official
                        }<br/> 
                        Superficie : ${country.area} km² <br/> 
                        Langues : ${lstLng.join(", ")} <br/> 
                        Monnaie : ${cur["name"]} <br/> 
                        Symbole : ${cur["symbol"]}`;
      } else {
        p.innerHTML = `${country.name.common}`;
        p2.innerHTML = `Capitale : Pas de capitale  <br/>  
                        Habitants : ${country.population} <br/> 
                        Nom en francais : ${
                          country.translations.fra.official
                        }<br/> 
